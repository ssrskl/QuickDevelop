...
@Mapper
public interface DqFollowUserPostProcessorMapper extends BaseMapper<DqUserPostProcessor> {
  /**
   * 查询指定用户关注的人
   *
   * @param dqUserId
  ...
  @Select("select dq_user.user_id,\n" +
          ...
          "from dq_user\n" +
          "         left join dq_user_role dur on dq_user.user_id = dur.user_id\n" +
          "where dq_user.user_id = any(select community.dq_follow_interdquser.give_follow_dquser_id\n" +
          "                         from dq_follow_interdquser\n" +
          "                         where followed_dquser_id = #{dqUserId})")
